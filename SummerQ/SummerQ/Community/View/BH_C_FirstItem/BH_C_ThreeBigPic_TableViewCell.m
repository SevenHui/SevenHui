...
//
//  BH_C_ThreeBigPic_TableViewCell.m
//  SummerQ
//
...
@implementation BH_C_ThreeBigPic_TableViewCell
-(void)dealloc{
    [_imageViewPhotoThree release];
    [_imageViewPhotoTwo release];
    [_imageViewphotoOne release];
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        // 图片1
        self.imageViewphotoOne = [[UIImageView alloc] init];
        [self.backView addSubview:_imageViewphotoOne];
        [self.imageViewphotoOne release];
        // 2
        self.imageViewPhotoTwo = [[UIImageView alloc] init];
        [self.backView addSubview:_imageViewPhotoTwo];
        [self.imageViewPhotoTwo release];
        // 3
        self.imageViewPhotoThree = [[UIImageView alloc] init];
        [self.backView addSubview:_imageViewPhotoThree];
        [self.imageViewPhotoThree release];
...
-(void)layoutSubviews{
    ...
    // 图片1
    self.imageViewphotoOne.frame = CGRectMake(WIDTHBACK * 0.15, HEIGHTBACK * 0.4, WIDTHBACK * 0.25, HEIGHTBACK * 0.3);
//    self.imageViewphotoOne.backgroundColor = COLOR;
    
    // 2
    self.imageViewPhotoTwo.frame = CGRectMake(WIDTHBACK * 0.42, HEIGHTBACK * 0.4, WIDTHBACK * 0.25, HEIGHTBACK * 0.3);
//    self.imageViewPhotoTwo.backgroundColor = COLOR;
    // 3
    self.imageViewPhotoThree.frame = CGRectMake(WIDTHBACK * 0.69, HEIGHTBACK * 0.4, WIDTHBACK * 0.25, HEIGHTBACK * 0.3);
//    self.imageViewPhotoThree.backgroundColor = COLOR;
...
-(void)setFirstItemModel:(BH_C_FirstItem_Model *)firstItemModel{
    ...
    // 图片
    [self.imageViewphotoOne sd_setImageWithURL:[NSURL URLWithString:self.firstItemModel.bigpic_arr[0]] placeholderImage:[UIImage imageNamed:@"占位图"]];
    [self.imageViewPhotoTwo sd_setImageWithURL:[NSURL URLWithString:self.firstItemModel.bigpic_arr[1]] placeholderImage:[UIImage imageNamed:@"占位图"]];
    [self.imageViewPhotoThree sd_setImageWithURL:[NSURL URLWithString:self.firstItemModel.bigpic_arr[2]] placeholderImage:[UIImage imageNamed:@"占位图"]];
