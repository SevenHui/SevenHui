//
//  BH_R_DisCount_SecondSection_TableViewCell.m
//  SummerQ
//
...
@implementation BH_R_DisCount_SecondSection_TableViewCell
-(void)dealloc{
...
-(void)setDiscountSecondModel:(BH_R_Discount_Model *)discountSecondModel{
    if (_discountSecondModel != discountSecondModel) {
        [_discountSecondModel release];
        _discountSecondModel = [discountSecondModel retain];
    }
    [self.imageViewPhoto sd_setImageWithURL:[NSURL URLWithString:self.discountSecondModel.photo] placeholderImage:[UIImage imageNamed:@"占位图"]];
    self.labelTitle.text = self.discountSecondModel.title;
    self.labelNum.text = self.discountSecondModel.sold;
    NSString *str = [NSString stringWithFormat:@"%@元起", self.discountSecondModel.price];
    NSString *keyOne = [NSString stringWithFormat:@"%@", self.discountSecondModel.price];
